<div style="margin-left: {{ min($depth, 4) * 28 }}px" class="{{ $depth > 0 ? 'mt-2' : 'mt-3' }}">
    ...
    <div id="reply-form-{{ $comment->id }}" class="hidden mt-2">
        <form action="{{ route('student-works.comment', $work) }}" method="POST" enctype="multipart/form-data" class="space-y-2 bg-white border border-outline-variant rounded-lg p-3">
            @csrf
            ...
            <div class="flex items-center gap-2">
                <label class="flex items-center gap-1 text-xs text-on-surface-variant cursor-pointer">
                    ...
                    <span class="reply-file-label-{{ $comment->id }}">Tambah Foto</span>
    ...
    @foreach ($comment->replies as $reply)
        @include('public.student-works.partials.comment', ['comment' => $reply, 'work' => $work, 'reasons' => $reasons, 'depth' => $depth + 1, 'isLoggedIn' => $isLoggedIn])
    @endforeach
</div>