@extends('layouts.dashboard')
@section('title', 'Profil Saya')

@section('dashboard-content')
<h1 class="font-headline text-2xl font-bold text-navy-deep mb-6">Profil Saya</h1>

@if (session('status'))
    <div class="mb-4 p-3 bg-status-success/10 text-status-success rounded-md text-sm">{{ session('status') }}</div>
@endif
@if ($errors->any())
    <div class="mb-4 p-3 bg-error-container text-status-error rounded-md text-sm">
        <ul class="list-disc list-inside">@foreach ($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul>
    </div>
@endif

<form method="POST" action="{{ route('guru.profile.update') }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-outline-variant/30 p-6 space-y-6 max-w-3xl">
    @csrf
    @method('PUT')

    <div class="flex items-center gap-4">
        <img src="{{ $teacher->photo ? asset('storage/'.$teacher->photo) : 'https://ui-avatars.com/api/?name='.urlencode($teacher->name) }}"
             class="w-20 h-20 rounded-full object-cover">
        <div>
            <label class="text-sm font-medium text-on-surface-variant block mb-1">Foto Profil</label>
            <input type="file" name="photo" accept="image/*" class="text-sm">
        </div>
    </div>

    <div>
        <label class="text-sm font-medium text-on-surface-variant">Email</label>
        <input type="email" name="email" value="{{ old('email', $teacher->email) }}" class="mt-1 w-full rounded-md border-outline-variant">
    </div>

    <div>
        <label class="text-sm font-medium text-on-surface-variant">Nomor WhatsApp</label>
        <input type="text" name="whatsapp_number" value="{{ old('whatsapp_number', $teacher->whatsapp_number) }}" placeholder="628xxxxxxxxxx" class="mt-1 w-full rounded-md border-outline-variant">
    </div>

    <div>
        <label class="text-sm font-medium text-on-surface-variant">Deskripsi Singkat</label>
        <textarea name="description" rows="3" class="mt-1 w-full rounded-md border-outline-variant">{{ old('description', $teacher->description) }}</textarea>
    </div>

    <div>
        <h3 class="font-headline text-navy-deep mb-3">Kelas & Jadwal Mengajar</h3>

        <div id="schedule-rows" class="space-y-2">
            @forelse ($schedules as $i => $sch)
            <div class="schedule-row bg-surface-container-low border border-outline-variant rounded-lg p-3">
                <div class="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto_auto_auto] gap-2 items-center">
                    <select name="schedules[{{ $i }}][class_id]" class="rounded-md border-outline-variant text-sm w-full">
                        @foreach ($classes as $c)
                            <option value="{{ $c->id }}" {{ $sch->class_id == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                        @endforeach
                    </select>
                    <select name="schedules[{{ $i }}][day]" class="rounded-md border-outline-variant text-sm w-full">
                        <option value="">Pilih Hari</option>
                        @foreach (['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'] as $d)
                            <option value="{{ $d }}" {{ $sch->day == $d ? 'selected' : '' }}>{{ $d }}</option>
                        @endforeach
                    </select>
                    <div class="flex items-center gap-1">
                        <span class="material-symbols-outlined text-on-surface-variant text-[18px] shrink-0">schedule</span>
                        <input type="time" name="schedules[{{ $i }}][start_time]" value="{{ $sch->start_time ? substr($sch->start_time, 0, 5) : '' }}"
                               class="rounded-md border-outline-variant text-sm w-full min-w-0">
                    </div>
                    <div class="flex items-center gap-1">
                        <span class="text-on-surface-variant text-sm shrink-0">-</span>
                        <input type="time" name="schedules[{{ $i }}][end_time]" value="{{ $sch->end_time ? substr($sch->end_time, 0, 5) : '' }}"
                               class="rounded-md border-outline-variant text-sm w-full min-w-0">
                    </div>
                    <button type="button" onclick="this.closest('.schedule-row').remove()"
                            class="flex items-center justify-center w-9 h-9 rounded-md text-status-error hover:bg-error-container/40 shrink-0 ml-auto sm:ml-0">
                        <span class="material-symbols-outlined text-[20px]">delete</span>
                    </button>
                </div>
            </div>
            @empty
            @endforelse
        </div>

        <button type="button" id="add-schedule"
                class="mt-3 flex items-center gap-1 text-sm font-bold text-math-teal hover:brightness-110">
            <span class="material-symbols-outlined text-[18px]">add_circle</span>
            Tambah Jadwal
        </button>
        <p class="text-xs text-on-surface-variant mt-1">
            Tambahkan sebanyak jadwal mengajar yang kamu perlukan. Kelas yang sama boleh muncul lebih dari sekali untuk hari yang berbeda.
        </p>
    </div>

    <button type="submit" class="bg-math-teal text-white px-6 py-3 rounded-md font-bold hover:brightness-110">Simpan Perubahan</button>
</form>
@endsection

@push('scripts')
<script>
let idx = {{ $schedules->count() }};
document.getElementById('add-schedule').addEventListener('click', () => {
    const classOptions = `@foreach ($classes as $c)<option value="{{ $c->id }}">{{ $c->name }}</option>@endforeach`;
    const dayOptions = `<option value="">Pilih Hari</option>` + `@foreach (['Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'] as $d)<option value="{{ $d }}">{{ $d }}</option>@endforeach`;

    const row = document.createElement('div');
    row.className = 'schedule-row bg-surface-container-low border border-outline-variant rounded-lg p-3';
    row.innerHTML = `
        <div class="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto_auto_auto] gap-2 items-center">
            <select name="schedules[${idx}][class_id]" class="rounded-md border-outline-variant text-sm w-full">${classOptions}</select>
            <select name="schedules[${idx}][day]" class="rounded-md border-outline-variant text-sm w-full">${dayOptions}</select>
            <div class="flex items-center gap-1">
                <span class="material-symbols-outlined text-on-surface-variant text-[18px] shrink-0">schedule</span>
                <input type="time" name="schedules[${idx}][start_time]" class="rounded-md border-outline-variant text-sm w-full min-w-0">
            </div>
            <div class="flex items-center gap-1">
                <span class="text-on-surface-variant text-sm shrink-0">-</span>
                <input type="time" name="schedules[${idx}][end_time]" class="rounded-md border-outline-variant text-sm w-full min-w-0">
            </div>
            <button type="button" onclick="this.closest('.schedule-row').remove()"
                    class="flex items-center justify-center w-9 h-9 rounded-md text-status-error hover:bg-error-container/40 shrink-0 ml-auto sm:ml-0">
                <span class="material-symbols-outlined text-[20px]">delete</span>
            </button>
        </div>`;
    document.getElementById('schedule-rows').appendChild(row);
    idx++;
});
</script>
@endpush